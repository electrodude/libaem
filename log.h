...
struct aem_log_dest {
	void (*log)(struct aem_log_dest *dst, struct aem_log_module *mod, enum aem_log_level level, struct aem_stringslice msg);
	enum aem_log_flags flags;
};
...
#define AEM_LOG_MULTI_BUF_MOD_IMPL(str, buf, module, loglevel, file, line, func) \
	for (struct aem_stringbuf *str = aem_log_header_mod_impl((buf), (module), (loglevel), (file), (line), (func)); \
	     str; aem_log_submit((aem_log_module_current), loglevel, str), str = NULL)
